...
class LiteSearchState(Generic[StateT]):
    ...
    exp_state: Dict[int, float] = field(default_factory=dict) 
    limit_state: Dict[int, float] = field(default_factory=dict) 
    leaf_node: list[int] = field(default_factory=list)

class LiteSearchIncremental(Algorithm[StateT, LiteSearchState[StateT]]):
    """
    Lite Search algorithm
    ...
    def __init__(
        ...
        *,
        ...
        self.lmbd = lmbd
        self.eps = eps
        print(f"Initialized LiteSearch Incremental with budget={self.budget}, lambda={self.lmbd}, epsilon={self.eps}")
    ...
    def step(
        ...
        """
        Perform one step of the LiteSearch algorithm.
        ...
        # state.exp_state[node_id] = 0
        state.limit_state[node_id] = self._calc_limit(state, new_node)
        state.leaf_node.append(node_id)

        parent = new_node.parent
        if len(parent.children) >= state.limit_state[parent.expand_idx]:
            state.leaf_node.remove(parent.expand_idx)
    ...
    def _select(self, state: LiteSearchState) -> Node:
        if len(state.leaf_node) == 0:
            ...
            return node
        if len(state.leaf_node) == 1:
            node = state.tree.get_node(state.leaf_node[0])
            return node
    
        best_leaf = max(state.leaf_node, key=lambda k: state.value_state[k])
        node = state.tree.get_node(best_leaf)
        return node
