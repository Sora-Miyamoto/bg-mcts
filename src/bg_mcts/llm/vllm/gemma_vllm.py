...
PRICING = {"google/gemma-3-4b-it": 1,
           "google/gemma-3-12b-it": 1}

class GemmaVllm(Model):
    def __init__(
        self,
        model_name: str,     
        model_setting: dict,
        ...
        if environment == "local":
            chat_template_path = f"{FOLDER_PATH}src/bg_mcts/llm/vllm/gemma_chat_template.jinja"
    ...
    def cut_messages(self, messages, max_tokens):
        ...
        if len_ids + max_tokens > max_model_len:
            pattern = self.tokenizer.encode("<start_of_turn>model\n")[1:] #[105, 4368, 107]
