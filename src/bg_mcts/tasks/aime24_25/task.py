...
from bg_mcts.data_types import Action, AIMEProbData
...
class AIMEProblem(Task):
    def __init__(self, data: AIMEProbData, prm_name: str, prm_setting: dict) -> None:
    ...
    @classmethod
    def load_file(cls, json_path: Path | str, prm_name: str, prm_setting: dict) -> "AIMEProblem":
        prob_path = Path(json_path)
        if not prob_path.exists():
            raise RuntimeError(f"AIME problem not found at {str(prob_path)}")
