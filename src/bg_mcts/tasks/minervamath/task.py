...
from bg_mcts.data_types import Action, MinervamathProbData
...
class MinervamathProblem(Task):
    def __init__(self, data: MinervamathProbData, prm_name: str, prm_setting: dict) -> None:
    ...
    @classmethod
    def load_file(cls, json_path: Path | str, prm_name: str, prm_setting: dict) -> "MinervamathProblem":
        prob_path = Path(json_path)
        if not prob_path.exists():
            raise RuntimeError(f"Minervamath problem not found at {str(prob_path)}")
